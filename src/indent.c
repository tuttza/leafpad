...
void indent_multi_line_indent(GtkTextBuffer *buffer)
{
	...
	if (gtk_text_iter_starts_line(&end_iter) && !gtk_text_iter_equal(&start_iter, &end_iter)) {
		end_line -= 1;
	}

	// Indent each line from start_line to end_line (inclusive)
	...
	// Restore the selection to include the indented lines
...
void indent_multi_line_unindent(GtkTextBuffer *buffer)
{
	...
	// Unindent each line from start_line to end_line (inclusive)
	for (i = start_line; i <= end_line; i++) {
	...
	// Restore the selection for unindented lines:
	gtk_text_buffer_get_iter_at_line(buffer, &start_iter, start_line);
	gtk_text_buffer_get_iter_at_line(buffer, &end_iter, end_line);
	gtk_text_iter_forward_to_line_end(&end_iter);
	gtk_text_buffer_select_range(buffer, &start_iter, &end_iter);
